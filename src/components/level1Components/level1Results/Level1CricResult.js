...
function Level1CricResult() {
  ...
  const claimCracks=()=>{
    localStorage.setItem('pbv1',35)
    if(score==5){
      localStorage.setItem('cracks',50)
    }
  ...
  const retryLevel=()=>{
    history.push('/level1/1')
  }
  const proceedToChapter2=()=>{

    if(cracksClaimed=="true"){
      localStorage.setItem('LevelInc',2)
        ...
        <div className="col s12 l12 ">
          <div className="col s12 l12 level1CricResultContainerCard">
            ...
            <div className="center-align" style={{marginTop:"35px"}}>
              {score==5?<div><i className="far fa-laugh-beam level1HappyEmoji"></i><p className="white-text">Wow ! you have successfully cracked Chapter 1</p></div>:<div><i className="far fa-frown level1SadEmoji"></i><p className="white-text">Better luck next time</p></div>}
            ...
            {score==5?<div>
            <a  onClick={claimCracks} className="btn-med yellow darken-3 btn-floating pulse" style={{marginTop:"5px",marginBottom:"8px"}}>Claim</a>
            </div>:''}
            </div>
            {score==5?<div className="center-align" style={{marginTop:"40px"}}><a onClick={proceedToChapter2}className="btn-small yellow darken-3">Unlock Chapter 2</a></div>:<div className="center-align" style={{marginTop:"40px"}}><a onClick={retryLevel} className="btn-small yellow darken-3">Retry Chapter 1</a></div>}
...
export default Level1CricResult
