...
function Level2SportsResult() {
  ...
  const claimCracks=()=>{
    localStorage.setItem('pbv2',35)
    if(score==7){
      localStorage.setItem('cracks',250)
    }
 M.toast({html: 'Successfully Claimed'})
  ...
  const retryLevel=()=>{
    ...
    setScore(0)
    history.push('/level2/1')
  }
  const proceedToChapter2=()=>{
    if(cracksClaimed=="true"){
      localStorage.setItem("level1Score",0)
    setScore(0)
      localStorage.setItem('Level2Inc',2)
        ...
        <div className="col s12 l12 ">
          <div className="col s12 l12 level1CricResultContainerCard">
            <div className="center-align">
              <h5 className="white-text">You Have Scored: <span className="level1CricResultScore">{score}/7</span></h5>
            </div>
            <div className="center-align" style={{marginTop:"35px"}}>
              {score==7?<div><i className="far fa-laugh-beam level1HappyEmoji"></i><p className="white-text">Wow ! you have successfully cracked Chapter 1</p></div>:<div><i className="far fa-frown level1SadEmoji"></i><p className="white-text">Better luck next time</p></div>}
            
            </div>
            <div className="center-align black" style={{borderRadius:"35px",marginTop:"35px",padding:"10px"}}>
            <p className="white-text" style={{marginTop:'5px'}}>You have earned <span style={{color:"#FFD700"}}>{score==7?100:0}</span> Cracks </p>
            ...
            {score==7?<div className="center-align" style={{marginTop:"40px"}}><a onClick={proceedToChapter2} href="" className="btn-small yellow darken-3">Unlock Chapter 2</a></div>:<div className="center-align" style={{marginTop:"40px"}}><a onClick={retryLevel} href="" className="btn-small yellow darken-3">Retry Chapter 1</a></div>}
...
export default Level2SportsResult
